...
- Fixes issue building with `-fsanitize=address` on macOS
...
- __BREAKING CHANGE__: `SELECT` statements (now also including schema independent queries like `SELECT 1`) cannot be executed on encrypted databases prior to setting the database key (behavior inherited from upstream SQLite)
...
- Updates `LICENSE.md` with SQLCipher license to avoid ambiguity and remove redundancy
...
- Fixes unfreed OpenSSL allocation when compiled against version 3
...
- Adds `PRAGMA cipher_log` and `PRAGMA cipher_log_level` features to allow logging of TRACE, DEBUG, INFO, WARN, and ERROR messages to stdout, stderr, file, or logcat
...
- Adds new `PRAGMA cipher_test_on`, `PRAGMA cipher_test_off`, and `PRAGMA cipher_test_rand` (available when compiled with `-DSQLCIPHER_TEST`) to facilitate simulation of error conditions
...
- Fixes `PRAGMA cipher_migrate` to clean up the temporary database if a migration fails
...
- Omits unused profiling function under certain defines to avoid compiler warnings
...
- Cryptographic provider interface cleanup
...
- Clear codec data buffers if a cryptographic provider operation fails
...
- Adds `PRAGMA cipher_compatibility` and `PRAGMA cipher_default_compatibility` which automatically configure appropriate compatibility settings for the specified SQLCipher major version number
...
- By default all memory allocated internally by SQLite is wiped before it is freed
- `PRAGMA cipher_memory_security`: allows full memory wiping to be disabled for performance when the feature is not required
- `PRAGMA cipher_kdf_algorithm`, `PRAGMA cipher_default_kdf_algorithm` to control KDF algorithm selection between PBKDF2-HMAC-SHA1, PBKDF2-HMAC-SHA256 and PBKDF2-HMAC-SHA512
- `PRAGMA cipher_hmac_algorithm`, `PRAGMA cipher_default_hmac_algorithm` to control HMAC algorithm selection between HMAC-SHA1, HMAC-SHA256 and PBKDF2-HMAC-SHA512
- Based on upstream SQLite 3.25.2
- When compiled with readline support, `PRAGMA key` and `PRAGMA rekey` lines will no longer be
  saved to history
...
- `PRAGMA cipher_plaintext_header_size` and `PRAGMA cipher_default_plaintext_header_size`: allocates a portion of the database header which will not be encrypted to allow identification as a SQLite database
...
- Fix `sqlcipher_export` and `PRAGMA cipher_migrate` when tracing API in use
